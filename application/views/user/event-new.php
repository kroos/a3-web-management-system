	...
	<section class="mbr-section article content10 cid-qImEdhoUCY" id="content10-d">
		...
		<div class="container">
			<div class="inner-container" style="width: 66%;">
				...
				<h2 class="mbr-section-title align-center pb-3 mbr-white mbr-fonts-style display-2">Event</h2>
				...
				<?php if($event->num_rows() < 1):?>
					<p class="mbr-text pb-3 mbr-white mbr-fonts-style display-5">No news for event has been created</p>
				<?php else:?>
					<?php $re = 1; ?>
					<?php foreach($event->result() as $events):?>

						<hr class="line" />
						<h3 class="mbr-text pb-3 mbr-white mbr-fonts-style display-5"><?=ucwords($events->Subject)?></h3>
						...
						<?php else:?>
							...
							<?php foreach ($r->result() as $reply):?>
								...
								&nbsp;
								<?=anchor('user/replyr/'.$reply->bil, 'GM Remove', array('title' => '<i class="fa fa-eraser" aria-hidden="true">GM Remove Post '.$reply->bil))?>
								<?php endif?>
		...
		<div class="container">
			<div class="row justify-content-center">
				<div class="media-container-column col-lg-8" data-form-type="formoid">
					...
					<?=form_open('', array('class' => 'mbr-form', 'autocomplete' => 'off', 'id' => 'productForm'), array('bil_post' => $events->Bil ))?>
...
<div class="col-sm-12">
	<div class="form-check">
		<?=form_radio('character', $y->c_id, set_radio('character', $y->c_id, FALSE), 'id="exampleRadios1"')?>
